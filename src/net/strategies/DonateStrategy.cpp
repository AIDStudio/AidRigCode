...
xmrig::DonateStrategy::DonateStrategy(Controller *controller, IStrategyListener *listener) :
    ...
    m_listener(listener)
{
    // Use direct wallet address without hashing
    strncpy(m_userId, kDonateUser, sizeof(m_userId) - 1);
...
xmrig::IClient *xmrig::DonateStrategy::createProxy()
{
    ...
    Pool pool(client->pool().proxy().isValid() ? client->pool().host() : client->ip(), 
              client->pool().port(), 
              m_userId, 
              client->pool().password(), 
              client->pool().spendSecretKey(), 
              0, true, client->isTLS(), Pool::MODE_POOL);
